...
app.MapGet("/tasks/complete", async (TaskDb db) => await db.Tasks.Where(t => t.IsComplete).ToListAsync());
app.MapGet("/tasks/{id}", async (int id, TaskDb db) => await db.Tasks.FindAsync(id) is Task task ? Results.Ok(task) : Results.NotFound());
